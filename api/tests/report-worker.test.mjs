...
const worker=load('src/lib/dailyDigestWorker.ts',{'./supabase':{},'./checkinPolicy':consent,'./familyAssistantPolicy':sharing,'./familyTestScope':{},'./dailyDigestPrivacy':{},'./dailyDigestWriter':{},'./dailyDigestPolicy':policy,'./reportDelivery':{}});
function setup({revoked=false,ambiguous=false,stale=false,ready=true,allowed=true,empty=false}={}){
 const sub={id:'sub',status:'active',buyer_phone_number:'+12025550123',senior_phone_number:'+12025550124'};
 const prefs={subscription_id:'sub',enabled:true,report_channel:'messages',calls_consent_at:'2026-09-01',reports_consent_at:'2026-09-01',recipient_consent_at:'2026-09-01',consent_senior_phone:sub.senior_phone_number,digest_timezone:'Europe/Prague',subscriptions:sub};
 ...
 const digest={id:'digest',recipient_phone:sub.buyer_phone_number,senior_phone:sub.senior_phone_number,status:'building',claim_token:'lease',consent_context:stamp};
 const events=empty?[]:['conversation','medication','water'].map((kind,i)=>({call_id:String(i),kind,answered:true,ended_at:'2026-09-15T17:00:00Z',available_at:'2026-09-15T17:01:00Z'}));
 const sends=[];let claimed=false;
 const db={rpc:async()=>{if(claimed)return {data:[]};claimed=true;return{data:[{...digest}]};},from(table){let patch=null,filters=[],single=false;const b={select(){return b},eq(k,v){filters.push([k,v]);return b},order(){return b},limit(){return b},maybeSingle(){single=true;return b},single(){single=true;return b},update(v){patch=v;return b},then(resolve,reject){let data=null;
  if(table==='daily_checkin_preferences')data=single?{...prefs,...(revoked?{reports_consent_at:null}:{})}:[prefs];
