-- Reports use the timezone chosen by the loved one, not the family proposal.
...
begin
  ...
  insert into public.daily_checkin_preferences(subscription_id,enabled,consent_senior_phone,calls_consent_at,schedule_confirmed_at,timezone)
    select s.id,true,caller_phone,now(),now(),caller_timezone from public.subscriptions s
    ...
    on conflict(subscription_id) do update set enabled=true,consent_senior_phone=excluded.consent_senior_phone,
      calls_consent_at=excluded.calls_consent_at,schedule_confirmed_at=excluded.schedule_confirmed_at,timezone=excluded.timezone,updated_at=now();
end;
$$;
